...
function top2(){
   	form3.action="<%=basePath%>admin/bmyy/s2info.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/bmyy/s2info.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/bmyy/s2info.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/bmyy/s2info.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/bmyy/s2info.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/bmyy/s2info.jsp?page="+page;
      form3.submit();
...
<div class="title_right"><strong>用户预约管理</strong></div>  
...
<table class="table table-bordered table-striped table-hover">
     <tbody>
       <tr align="center">
         ...
         <td nowrap="nowrap"><strong>主题</strong></td>
         <td nowrap="nowrap"><strong>日期</strong></td> 
         <td nowrap="nowrap"><strong>人员</strong></td>
         <td nowrap="nowrap"><strong>主持人</strong></td>
         <td nowrap="nowrap"><strong>类型</strong></td>
         <td nowrap="nowrap"><strong>会议室名</strong></td> 
         <td nowrap="nowrap"><strong>容纳人数</strong></td> 
         <td nowrap="nowrap"><strong>所在楼层</strong></td>
         <td nowrap="nowrap"><strong>多媒体设备</strong></td>
...
<%String lx=Common.toChineseAndTrim(request.getParameter("lx")); 
String word=Common.toChineseAndTrim(request.getParameter("word")); 
String rq=Common.toChineseAndTrim(request.getParameter("rq"));
	cb.setEVERYPAGENUM(12);
	int cou = cb.getMessageCount("select count(*) from bmyy where "+lx+" like '%"+word+"%' and rq = '"+rq+"' ");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from bmyy  where "+lx+" like '%"+word+"%' and rq = '"+rq+"'  order by id desc",14);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap"> 
         <a href="<%=basePath%>admin/bmyy/sh.jsp?method=shbmyy&id=<%=pagelist2.get(0).toString()%>">审核</a>
         <a href="<%=basePath%>ComServlet?method=delbmyy2&id=<%=pagelist2.get(0).toString()%>">删除</a>
         </td>
       </tr>
<% }} %>
       <tr align="center">
         <td nowrap="nowrap" colspan="19"><input type="hidden" name="lx" value="<%=lx%>" />
